...
- **`LlmClientFactory`** — 从 `agentic.providers` 配置读取 provider 配置，通过 Hyperf DI 注入 `LlmClient`
- **`AgenticFactory`** — 从 `agentic.agents` 和 `agentic.agentic` 配置读取 Agent 定义，通过 Hyperf DI 注入 `Agentic`

### 变更

- `ConfigProvider` 将 `LlmClient` 和 `Agentic` 映射到工厂类而非直接类解析。`vendor:publish` 现在开箱即用——无需手动覆盖 DI 绑定
